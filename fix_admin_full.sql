...
CREATE POLICY "agenda_users_update_admin" ON public.agenda_users
FOR UPDATE
USING ( get_is_admin_safe() = true );
...
WHERE id IN (
    SELECT id 
    FROM auth.users 
    WHERE email = 'mauro.rementeria@tmoviles.com.ar' -- <--- Corregido al email de la empresa
);
