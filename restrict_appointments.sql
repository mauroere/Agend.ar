...
CREATE POLICY agenda_tenant_insert_appointments ON public.agenda_appointments FOR
INSERT
WITH
    CHECK (
...
CREATE POLICY agenda_tenant_delete_appointments ON public.agenda_appointments FOR DELETE USING (
    tenant_id = public.agenda_current_tenant_id ()
);

-- UPDATE: Allow updating (similar to delete)
CREATE POLICY agenda_tenant_update_appointments ON public.agenda_appointments FOR
UPDATE USING (
    tenant_id = public.agenda_current_tenant_id ()
)
WITH
...
CREATE POLICY agenda_tenant_select_appointments ON public.agenda_appointments FOR
SELECT USING (
        tenant_id = public.agenda_current_tenant_id ()
        AND (
            ...
            provider_id IN (
                SELECT id
                FROM public.agenda_providers ap
                WHERE
                    ap.user_id = auth.uid ()
            )
            OR
