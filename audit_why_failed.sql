...
SELECT 
    ...
    CASE WHEN pu.id IS NOT NULL THEN 'SI' ELSE 'NO' END as en_agenda_users,
    pu.tenant_id as tenant_actual_user,
    t1.name as nombre_tenant_user,
    -- Si existe como provider vinculado
    CASE WHEN pp.id IS NOT NULL THEN 'SI' ELSE 'NO' END as es_provider_vinculado,
...
SELECT * FROM public.agenda_providers 
WHERE full_name ILIKE '%Mauro%' OR full_name ILIKE '%Rementeria%';
