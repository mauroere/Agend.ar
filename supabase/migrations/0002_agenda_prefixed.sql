...
-- Indexes
create index if not exists agenda_idx_appointments_tenant_location_start on public.agenda_appointments (
    tenant_id,
    location_id,
    start_at
);
...
-- Policies
create policy agenda_tenant_select_tenants on public.agenda_tenants for
select using (
        id = public.agenda_current_tenant_id ()
    );

create policy agenda_tenant_select_locations on public.agenda_locations for
select using (
        tenant_id = public.agenda_current_tenant_id ()
    );

create policy agenda_tenant_insert_locations on public.agenda_locations for
insert
with
    check (
        tenant_id = public.agenda_current_tenant_id ()
...
create policy agenda_tenant_access_users on public.agenda_users for
select using (
        tenant_id = public.agenda_current_tenant_id ()
    );
...
create policy agenda_tenant_access_message_log_select on public.agenda_message_log for
select using (
        tenant_id = public.agenda_current_tenant_id ()
    );

create policy agenda_tenant_access_message_log_insert on public.agenda_message_log for
insert
with
    check (
        tenant_id = public.agenda_current_tenant_id ()
...
DO $$ begin perform public.agenda_current_tenant_id ();

exception when others then raise exception 'JWT must include tenant_id claim';
