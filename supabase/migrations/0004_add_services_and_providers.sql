...
create policy agenda_tenant_access_services on public.agenda_services for all using (
...
create policy agenda_tenant_access_providers on public.agenda_providers for all using (
    tenant_id = public.agenda_current_tenant_id ()
)
with
    check (
...
create index if not exists agenda_idx_services_tenant_active on public.agenda_services (tenant_id, active, sort_order);

create index if not exists agenda_idx_providers_tenant_active on public.agenda_providers (
    tenant_id,
    active,
    default_location_id
);
