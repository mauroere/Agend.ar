...
create policy "Medical Records are viewable by tenant users"
  ...
  using ( tenant_id = (select auth.uid()::uuid) );
-- NOTE: This relies on auth.uid() matching user_id which usually links to tenant?
...
create policy "Enable all access for authenticated users of same tenant" on public.agenda_medical_records using (
    exists (
        select 1
        from public.agenda_users au
        where
            au.id = auth.uid ()
...
create policy "Enable all access for attachments of same tenant records" on public.agenda_medical_attachments using (
    exists (
        select 1
        from public.agenda_medical_records mr
            join public.agenda_users au on au.tenant_id = mr.tenant_id
        where
            mr.id = public.agenda_medical_attachments.record_id
            and au.id = auth.uid ()
    )
);
